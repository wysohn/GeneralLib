...
public class Decompose {
     ...
     *
     * Contains a lower matrix L, and upper matrix U, and a permutation matrix P
     * such that P*L*U is the original matrix.
     * 
     * @param <T>
     */
     ...
     *
     * Computes the LU decomposition using GETRF. Returns three matrices L, U,
     * P, where L is lower diagonal, U is upper diagonal, and P is a permutation
    ...
    public static FloatMatrix cholesky(FloatMatrix A) {
        ...
        if (info < 0) {
            ...
            throw new LapackPositivityException("DPOTRF",
                    "Minor " + info + " was negative. Matrix must be positive definite.");
        }
        clearLower(result);
        return result;
    }
     ...
     * QR decomposition.
     *
     * Decomposes (m,n) matrix A into a (m,m) matrix Q and an (m,n) matrix R
     * such that Q is orthogonal, R is upper triangular and Q * R = A
     *
     * Note that if A has more rows than columns, then the lower rows of R will
     * contain only zeros, such that the corresponding later columns of Q do not
     * enter the computation at all. For some reason, LAPACK does not properly
